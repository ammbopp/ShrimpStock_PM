...
const RequestCart = () => {
  ...
  useEffect(() => {
    ...
    if (location.state?.cart && location.state.cart.length > 0) {
      ...
      localStorage.setItem("requestCart", JSON.stringify(location.state.cart));
    } else {
      setCart(savedCart);
    }
  }, [location.state]);
  ...
  const removeItem = (index) => {
    ...
    setCart(newCart);
    // Update localStorage
    localStorage.setItem("requestCart", JSON.stringify(newCart));
